...
function printData(kategorie) {
    tutArr = getTutorialsZuKategorie(kategorie.name);


    for (let i = 0; i < tutArr.length; i++) {
        tut = tutArr[i];
        let printString = "Kategorie:" + kategorie.name + "\n"
            + "Bild:" + kategorie.bild().name + "\n";
        + tut.name + " (" + tut.sprache + ") " + tut.datum + " \n"
            + tut.beschreibung + "\n"
            + tut.getDauerInStundenUndMinuten(tut.dauer) + "\n"
            + tut.url + "\n"
        console.log(printString);

        for (let j = 0; j < tut.kapitel.length; j++) {
            console.log(printKapData(tut.kapitel[j]));
        }
    }
